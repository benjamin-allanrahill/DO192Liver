...
gene.names <- read.csv("/Users/c-allanb/Desktop/Benjamin_Allan-Rahill/D0192Liver/Data/Input/aa-tRNA_Sequence_Names.csv", header = FALSE)
```
...
scans_path <- '/Users/c-allanb/Desktop/Benjamin_Allan-Rahill/D0192Liver/Data/Output/'
...
for (f in scan_file_names){
  ...
  # Set the working directory to the '/Output/Plots' folder 
  setwd(paste(scans_path, "Plots", sep= ""))

  # Make a new folder for the gene and its plots
  dir.create(paste(scans_path, "Plots", f, sep ="" ))
  ...
  setwd(paste(scans_path, "Plots", f, sep ="" ))
  
  # Establish the graphical device that base R will send the plot to 
  png(paste(f, "_RNA", sep =""), height = 780, width = 780 )
  
  # Set the number of rows to 3 and columns to 1
  par(mfrow= c(3,1))
  
  # Create 3 plots with 3 threshold lines
  plot(x = lod.rna.addcovariatesonly, map = map, main = paste(f, "~Sex + Diet + Tag", "RNA"))
  abline(h = 4, col = "red", lwd = 2)
  
  plot(x = lod.rna.dietint, map = map, main = paste(f, "Diet Int", "RNA"))
  abline(h = 4, col = "red", lwd = 2)
  abline(h = 7, col = "yellow", lwd = 2)
  
  plot(x = lod.rna.sexint, map = map, main = paste(f, "Sex Int", "RNA"))
  abline(h = 4, col = "red", lwd = 2)
  abline(h = 7, col = "yellow", lwd = 2)
  ...
  # Establish the graphical device that base R will send the plot to 
  png(paste(scans_path, "Plots", f, "_Protein", sep ="" ))
  
  # Set the number of rows to 3 and columns to 1
  par(mfrow= c(3,1))
  
  # Create 3 plots with 3 threshold lines
  plot(x = lod.protein.addcovariatesonly, map = map, main = paste(f, "~Sex + Diet + Tag", "Protein"))
  abline(h = 4, col = "red", lwd = 2)
  
  plot(x = lod.protein.dietint, map = map, main = paste(f, "Diet Int", "Protein"))
  abline(h = 4, col = "red", lwd = 2)
  abline(h = 7, col = "yellow", lwd = 2)
  
  plot(x = lod.protein.sexint, map = map, main = paste(f, "Sex Int", "Protein"))
  abline(h = 4, col = "red", lwd = 2)
  abline(h = 7, col = "yellow", lwd = 2)
  abline(h = 10, col = "black", lwd = 2)
